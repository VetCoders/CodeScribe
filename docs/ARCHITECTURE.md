...
| **L2 — Lexicon + Light+**    | Deterministic authorized relabeling    | `admit_ledger_label` records the Lexicon observation (the former `core/pipeline/light_plus.rs` shaping surface is gone — do not restore it), and `custom_lexicon_entries` is the persisted custom-lexicon loading path |
