...
impl CodescribeConfig {
    ...
    pub fn remove_custom_provider(&self, id: String) -> Result<CsCustomProviderRemoval, CsError> {
        ...
        let removed = settings
            .remove_custom_provider(custom_id(&id))
            .map_err(provider_error)?;
        ...
        Ok(CsCustomProviderRemoval {
            id: ProviderRef::Custom(removed.provider.id).as_string(),
            lanes_reset: removed
                .lanes_reset
                .into_iter()
                .map(|lane| match lane {
    ...
    pub fn discover_models(&self, provider_id: String) -> CsModelDiscovery {
        ...
        let api_key = keychain::runtime_key(&provider.key_account);
